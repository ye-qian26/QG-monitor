...
public class UserController {
    ...
    public Result loginByPassword(@RequestBody EncryptedRequestDTO request) {
        try {
            ...
            if (map == null) {
                return new Result(Code.NOT_FOUND, "用户未注册或密码错误");
            }
            log.info("用户信息: {}", map);
            Users user = (Users) map.get("user");
            if (user == null) {
                return new Result(Code.BAD_REQUEST, "未注册");
            }
            ...
            return new Result(Code.SUCCESS, encryptionResultDTO, "登录成功");
        } catch (Exception e) {
    ...
    public Result register(@RequestBody EncryptedRequestDTO request) {
        ...
        try {
            ...
            if (registerDTO == null) {
                ...
                return new Result(Code.BAD_REQUEST, "注册信息不能为空");
            }

            if (registerDTO.getUsers() == null) {
                log.warn("注册失败，用户信息为空");
                return new Result(Code.BAD_REQUEST, "用户信息不能为空");
            }
            ...
            if (code.isEmpty()) {
                ...
                return new Result(Code.BAD_REQUEST, "验证码不能为空");
            }
            ...
            return new Result(Code.INTERNAL_ERROR, "注册过程中发生异常: " + e.getMessage());
        }
    }
    ...
    public Result updateAvatar(@RequestParam("avatar") MultipartFile file,
        ...
        try {
            // 验证文件是否为空
            if (file.isEmpty()) {
                return new Result(Code.BAD_REQUEST, "请选择有效的头像文件");
            }
            ...
            if (file.getSize() > 2 * 1024 * 1024) {
                return new Result(Code.BAD_REQUEST, "图片大小不能超过2MB");
            }

            log.info("file ==> {}", file.getOriginalFilename());
            String avatarUrl = FileUploadHandler.saveFile(file, FileUploadHandler.IMAGE_DIR);

            // 判断头像是否上传成功返回相应的结果
            if (usersService.updateAvatar(userId, avatarUrl)) {
                log.info("上传头像成功，url: {}", avatarUrl);
                return new Result(Code.SUCCESS, avatarUrl, "头像上传成功");
            } else {
            ...
            return new Result(Code.INTERNAL_ERROR, "头像上传失败");
        }
    }
