...
import com.qg.backend.mapper.SourcemapFilesMapper;
...
import com.qg.backend.service.AllErrorService;
...
//            );
            List<Responsibility> allResponsibilities = alertClient.getResponsibilityListByProjectId(projectId, null);
    ...
    public Result selectErrorDetail(Long errorId, String platform) {
        ...
        try {
            return switch (platform) {
                case "backend" -> {
                    BackendError backendError = backendErrorMapper.selectOne(
                            new LambdaQueryWrapper<BackendError>().eq(BackendError::getId, errorId));
                    yield new Result(SUCCESS, backendError, "查询成功");
                }
                case "frontend" -> {
                    FrontendError frontendError = frontendErrorMapper.selectOne(
                            new LambdaQueryWrapper<FrontendError>().eq(FrontendError::getId, errorId));
                ...
                case "mobile" -> {
                    MobileError mobileError = mobileErrorMapper.selectOne(
                            new LambdaQueryWrapper<MobileError>().eq(MobileError::getId, errorId));
