...
public interface FrontendErrorMapper extends BaseMapper<FrontendError> {
    ...
    @Select("""
        ...
        FROM (
          SELECT
            error_crumb->>'message' AS label,
            COUNT(*) AS value
          FROM (
            SELECT 
              jsonb_array_elements(breadcrumbs) AS error_crumb
            FROM pmp.frontend_error
            WHERE project_id = #{projectId}
              AND timestamp BETWEEN #{startTime} AND #{endTime}
          ) AS error_expanded
          WHERE error_crumb->>'captureType' = 'manual'
          GROUP BY error_crumb->>'message'
          
          UNION ALL
          
          SELECT
            behavior_crumb->>'message' AS label,
            COUNT(*) AS value
          FROM (
            SELECT 
              jsonb_array_elements(breadcrumbs) AS behavior_crumb
            FROM pmp.frontend_behavior
            WHERE project_id = #{projectId}
              AND timestamp BETWEEN #{startTime} AND #{endTime}
          ) AS behavior_expanded
          WHERE behavior_crumb->>'captureType' = 'manual'
          GROUP BY behavior_crumb->>'message'
        ) AS combined
        GROUP BY label
        """)
