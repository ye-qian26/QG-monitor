...
import com.qg.backend.domain.vo.MobileOperationVO;
...
public class MobilePerformanceServiceImpl implements MobilePerformanceService {
    ...
    public Result getAverageTime(String projectId, String timeType) {
        ...
        if (timeType == null || timeType.isEmpty()) {
            return new Result(Code.BAD_REQUEST, "时间类型不能为空");
        }
        ...
        switch (timeType) {
            ...
            case "month":
                ...
                break;
            default:
                return new Result(Code.BAD_REQUEST, "不支持的时间类型");
        }
        ...
        if (mobilePerformances.isEmpty()) {
            return new Result(Code.NOT_FOUND, "没有找到相关数据");
        }

        return new Result(Code.SUCCESS, averageTimeMap, "查询成功");
    }
