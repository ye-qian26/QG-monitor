...
public class FrontendErrorRepository extends ErrorRepository<FrontendError> {
    ...
    @Autowired
    public FrontendErrorRepository(StringRedisTemplate stringRedisTemplate, ProjectClient projectClient, RestTemplateBuilder restTemplateBuilder, UserClient userClient, AlertClient alertClient) {
     ...
     */
    @Override
    protected String generateAlertMessage(FrontendError error) {
        return String.format("【前端端错误告警】\n" +
                             "项目ID：%s\n" +
                             "错误类型：%s\n" +
                             "发生次数：%d\n" +
                             "触发时间：%s\n" +
     ...
     */
    @Override
    protected boolean shouldAlert(String redisKey, FrontendError error) {
    ...
    protected boolean checkNotificationNoExist(FrontendError error, LocalDateTime timestamp) {
        // 检测通知是否已存在
        log.info("错误信息：{}", error);
...
//        Notification notification = alertClient.getNotificationByWrapper(queryWrapper);
        Notification notification = alertClient.getNotification(error.getProjectId(),
                error.getErrorType(), "frontend", error.getId(), ALERT_CONTENT_NEW);
...
//
                    alertClient.updateResponsibility(error.getProjectId(), error.getErrorType(), "frontend", error.getId());
...
//                    alertClient.updateResponsibilityByWrapper(responsibility, queryWrapper);
                    alertClient.signResponsibilityNoHandle(error.getProjectId(), error.getErrorType(), "frontend");
...
//                    alertClient.updateResponsibilityByWrapper(responsibility1, queryWrapper5);
    ...
    @Override
    protected boolean saveNotification(List<Long> alertReceiverID, FrontendError error) {
        log.info("存进通知表！");
        ...
        for (Long receiverID : alertReceiverID) {
            ...
            notification.setPlatform("frontend");
