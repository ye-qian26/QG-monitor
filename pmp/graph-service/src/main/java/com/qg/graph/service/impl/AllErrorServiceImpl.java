...
public class AllErrorServiceImpl implements AllErrorService {
    ...
    public Result selectByCondition(String projectId, String type) {
        ...
        try {
            ...
            // 执行查询
            List<BackendError> backendErrors = backendClient.getBackendErrorByWrapper(backendQueryWrapper);
            List<FrontendError> frontendErrors = frontendClient.getFrontendErrorByWrapper(frontendQueryWrapper);
            List<MobileError> mobileErrors = mobileClient.getMobileErrorByWrapper(mobileQueryWrapper);

            // 查询责任人信息（一次性查询，避免在循环中多次查询）
            List<Responsibility> allResponsibilities = alertClient.getResponsibilityListByWrapper(
    ...
    public Result selectById(Long id) {
        ...
        BackendError backendError = backendClient.getBackendErrorByWrapper(backendQueryWrapper).getFirst();
        FrontendError frontendError = frontendClient.getFrontendErrorByWrapper(frontendQueryWrapper).getFirst();
        MobileError mobileError = mobileClient.getMobileErrorByWrapper(mobileQueryWrapper).getFirst();
    ...
    public Result selectErrorDetail(Long errorId, String platform) {
        ...
        try {
            return switch (platform) {
                case "backend" -> {
                    BackendError backendError = backendClient.getBackendErrorByWrapper(
                            new LambdaQueryWrapper<BackendError>().eq(BackendError::getId, errorId)).getFirst();
                    yield new Result(SUCCESS, backendError, "查询成功");
                }
                case "frontend" -> {
                    FrontendError frontendError = frontendClient.getFrontendErrorByWrapper(
                            new LambdaQueryWrapper<FrontendError>().eq(FrontendError::getId, errorId)).getFirst();
                    ...
                    SourcemapFiles sourcemapFiles = frontendClient.selectOneSourcemapFileByQueryWrapper(sourcemapQueryWrapper);
                ...
                case "mobile" -> {
                    MobileError mobileError = mobileClient.getMobileErrorByWrapper(
                            new LambdaQueryWrapper<MobileError>().eq(MobileError::getId, errorId)).getFirst();
